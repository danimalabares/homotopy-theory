...
\end{equation}
The hardest part is showing that a generator of $\pi_{2k-1}(\Omega^{2} S^{2k+1})$, which is a map $\beta:S^{2k-1}\to \Omega^{2} S^{2k+1}$, induces an isomorphism mod torsion on $H_{*}$, {\color{magenta}which is just another way of saying it is an isomorphism on $H_{*}(-;\mathbb{Q})$}.
