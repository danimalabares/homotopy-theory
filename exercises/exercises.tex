...
\[0=F_{-1}H_{n}\subset F_{0}H_{n}\subset \ldots\subset F_{n}H_n=H_n\]
such that
\[E^{\infty}_{p,q}=F^{p} H^{p+q} /F^{p-1} H^{p+q}.\]
But the 4th and 5th homology groups are zero, so the filtration is zero and so are the groups $E^{\infty}_{p,q}$ with $p+q=4,5$.
