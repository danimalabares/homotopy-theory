...
\begin{defn}\leavevmode
	\begin{itemize}
		...
		\end{thm}
		\item Let $A$ be a subspace of $X$ and $x_0\in A$. The elements of the \textbf{\textit{relative homotopy group $\pi_n(X,A,x_0)$}} are homotopy classes of maps $(I^n,\partial I^n,J^{n-1})\to (X,A,x_0)$ where $J^{n-1}$ is the union of all but one face of $I^n$.
		...
		Recall that, in general, a homotopy of maps $H_t:X\to Y$ is called \textbf{\textit{relative to $A\subset X$}} if $H_t|_A$ is constant.
		\begin{remark}
		...
		\[\begin{tikzcd}[column sep=small]
			\pi_n(A,x)\arrow[r]&\pi_{n}(X,x)\arrow[r]&\pi_{n-1}(A,x)\arrow[r]&\pi_{n-1}(X,x)\arrow[r]&\cdots\arrow[r]&\pi_0(X,x)
		\end{tikzcd}\]
		\item $n$-connected spaces, $n$-connected pair, $n$-equivalence.
	\end{itemize}
\end{defn}
