...
We know that cohomology of $\C P^n$ is …

So that
\begin{align*}
	H^\bullet(\C P^n)&=H^0(\C P^2)\oplus H^1(\C P^2)\oplus\ldots\oplus H^{2n}(\C P^{n})\\
	&=\Z\oplus\Z a_1\oplus\ldots\oplus\Z a_{n}
\end{align*}
We wish to show that 
\[a_1=\alpha,a_2=\alpha^2\ldots a_{n}=\alpha^n\]
for the generator of the cohomology group of degree 2 $\alpha$.
To show that $H^\bullet(\C P^n,\Z)\approx \Z[\alpha]/(\alpha^{n+1})$ with $\deg\alpha=2$ we use induction.

By Poincaré duality on $\C P^2$
\begin{align*}
	H^2(\C P^2)\times H^2(\C P^2)&\to H^4(\C P^2)\\
	(\alpha,\alpha)&\mapsto\alpha\smile\alpha =\alpha^2
\end{align*}
must map a generator to a generator, so the case of $\C P^2$ is done.

Now the case of $\C P^3$ is as follows:
\begin{align*}
		H^2(\C P^3)\times H^4(\C P^3)&\to H^6(\C P^3)\\
	(\alpha,\alpha^2)&\mapsto\alpha\smile\alpha^2=\alpha^3
\end{align*}
Notice we have used the isomorphism $H^i(\C P^3)\approx H^i(\C P^2)$ when choosing generators of each of the groups.

So we have the ring morphism
\begin{align*}
	\varphi:\Z[\alpha]&\to H^\bullet(\C P^2)=\Z\oplus\Z\alpha\oplus\ldots\oplus\Z\alpha^n\\
	\sum\lambda_i\alpha^i&\mapsto \bigoplus\lambda_i\alpha^i
\end{align*}
with
\[\ker\varphi=\{\}\]
